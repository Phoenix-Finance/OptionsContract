...
contract('OptionsManager', function (accounts){
  ...
  it('create new options token', function (){
    ...
    return OptionsManager.deployed().then(function(instance){
        ...
        await managerInstance.createOptionsToken("options token 1",whiteList[0],1,200,Math.floor(Date.now()/1000)+50,0);
        optionsList = await managerInstance.getOptionsTokenList();
        console.log(optionsList);
        await managerInstance.createOptionsToken("options token 1",whiteList[0],2,200,Math.floor(Date.now()/1000)+10000,0);
        optionsList = await managerInstance.getOptionsTokenList();
        console.log(optionsList);
